 ...
 * Template Name: DMCA
 *
 * Phase 8-B — page template for Digital Millennium Copyright Act
 * notice + takedown procedure. Theme renders a default heading + slot
 * for operator-supplied legal text. NEITHER KTUBE NOR THIS TEMPLATE
 * AUTHOR LEGAL TEXT — operators supply their DMCA agent contact
 * details, designated-agent registration paragraphs, and contact form.
 *
 * Auto-applied to any Page whose slug is `dmca` via
...
<main id="primary" class="site-main ktube-single ktube-single--page ktube-compliance-page ktube-compliance-page--dmca">
	<?php
	while ( have_posts() ) :
		the_post();
		$ktube_page_title      = get_the_title();
		$ktube_default_heading = function_exists( 'ktube_compliance_default_heading' )
			? ktube_compliance_default_heading( 'dmca' )
			: __( 'DMCA', 'ktube' );
