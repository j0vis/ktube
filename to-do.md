...
> Last updated: 2026-06-21 (+ re-audit pass against the original KingTube-rebuild brief). **Phase 0-A tech-stack reversal ships (hand-authored CSS, vendored Video.js, zero `npm install`)**; **Phase 7b Privacy disclosure ships**; **Phase 8 Customizer + dark-mode cleanup ships**; **Phase 9 WPS-compat audit ships**. This is the single source of truth for resuming work on the ktube WordPress theme across phase boundaries.

> 🎯 **Quick-jump:** the **consolidated remaining-task list** is at the top of §5 below (§5-A "Critical divergences" + §5-B "Secondary polish"). Eight items ship-block the Phase 11 hand-off — five are spec-mismatch or missing artifacts, three are infra/governance gates. Detailed cross-reference of every numbered spec item to ktube's current behavior lives in §0 below.
...
| Custom meta matching WPS Mass Importer's actual field map (duration, embed URL, quality variants, trailer asset, view count) | ⚠️ PARTIAL. Meta is registered (`includes/meta.php` declares both `_ktube_*` canonical keys AND the unprefixed legacy keys importer writes). Field map declared in `includes/wps-compat/mass-importer.php::ktube_importer_key_map()`. Sync-on-save via `importer-adapter.php`. **Still pending:** explicit DB-index recommendations doc for primary sort/filter at 10k+ scale (Phase 1-A in §5-A). |
...
| 3.7 | Mass Importer field-map cross-check + indexed DB columns for primary sort filters | ⚠️ PARTIAL. **Field map ships** — `includes/wps-compat/mass-importer.php::ktube_importer_key_map()` declares 13 legacy→ktube meta-key pairs (verified-against-plugin-source header); `includes/wps-compat/importer-adapter.php` syncs on `save_post_video`. **DB-index recommendations doc still missing** — Phase 1-A in §5-A captures the residual work. |
| 3.7 | Storyboard sprite previews (WebP/GIF, sprite-style scrub) | ❌ MISSING. Currently single-frame trailer swap. |
...
| 3.8 | 2257-style compliance pages (18 U.S.C 2257, DMCA, Privacy, Terms) — page templates / footer menu slots, NOT authored legal text | ⚠️ PARTIAL. **Privacy** shipped via Phase 9b (`page-privacy.php` + auto-apply template + Customizer `ktube_privacy_page_id` selector + footer badge). **2257 / DMCA / Terms** MISSING — Phase 8-B in §5-A. |
...
| Spec requirement | Status |
| --- | --- |
| PSI lab test 100/100/100 (Performance / Accessibility / Best Practices) on stock demo content, no plugins active | ❌ NO BASELINE RUN. We never invoked PSI against the theme. **Phase 14-A in §5 is BLOCKING for §4 closeout.** |
...
| Spec requirement | Status |
| --- | --- |
| `VideoObject` JSON-LD on single video (duration / thumbnail / uploadDate / embed URL) | ❌ MISSING. Article + ImageGallery + OG/Twitter cards shipped but no VideoObject. **Phase 9d in §5 — BLOCKING for video rich-snippets.** |
...
| Editable H1/description for the homepage front-page template field | ❌ MISSING. **Phase 8-A in §5.** |
...
| Spec requirement | Status |
| --- | --- |
| PSI run against stock-content install + scores in PR description | ❌ NOT DONE. Phase 14-A. |
...
| Mass Importer field map cross-checked against `includes/wps-compat/mass-importer.php` | ❌ NOT DONE (BLOCKING, brief §5.7). |
...
| Spec requirement | Status |
| --- | --- |
| Repo `https://github.com/j0vis/ktube` — main always deployable | ⚠️ Local repo only; **NOT pushed**. Phase 11-A in §5. |
...
| GitHub Actions CI step calling PSI REST API directly (no Lighthouse npm package) | ❌ NOT DONE. Phase 14-B in §5. |
...
| No invented meta-key schema diverging from Mass Importer contract | ❌ DEVIATION pending §5.7 audit (BLOCKING). |
...
2. ⚠️ **Mass Importer field map cross-check** — §5.7 hard-requirement; must read the importer's plugin source before any further Mass-Importer-adjacent change.
3. ⚠️ **RTA label format** — change `<meta http-equiv="RTA">` to `<meta name="rating" content="RTA-5042-1996-1400-1577-RTA">` to match ASACP register.
4. ⚠️ **VideoObject JSON-LD on video singles** — required for video rich-snippet eligibility.
5. ⚠️ **2257 / DMCA / Terms page templates** — replicate slots; do NOT author legal text.
6. ⚠️ **Editorial H1/description for the homepage** — needs Customizer field.
7. ⚠️ **PSI baseline run** — verify 100/100/100 budget on stock demo.
8. ⚠️ **Repo push to github.com/j0vis/ktube + conventional-commits + GH Actions PSI workflow**.

Items 1–8 must be resolved before the Phase 11 hand-off can be claimed.
...
## 3. Open / Pending — Reviewer-Flagged Nits Not Yet Closed

> Nits closed in the 2026-06-21 audit pass are detailed under **Phase 8** above; only surviving items are listed below.

### Customizer (handle-enqueued guard)
* `ktube_print_inline_customizer_vars()` checks `wp_style_is('ktube-main','registered')`. Should also test `'enqueued'` so a vulnerable handle that registered but failed to enqueue (e.g. dequeued by MU-plugin) doesn't emit orphan inline `<style>`.

### Schema (Article richness)
* `ktube_render_blog_article_schema()` doesn't include `articleBody` (use `wp_strip_all_tags the_content`) or `keywords` (from `post_tag` terms). Add for richer rich-snippets.
...
## 4. Upcoming Phases (from the brief)

### Phase 7b — Privacy disclosure
* Footer surface badge: "We verify visitors are 18+" → links to `/privacy` page.
* Privacy page documents: localStorage key, cookie name, retention period, RTA tag presence.
...
> As of 2026-06-21, items 1–6 from the prior brief are shipped (Phase 7 / 7b / 8 / 9b above). Section §0 (Specification gap audit) catalogs 8 critical divergences against the original KingTube-rebuild brief — those land at the TOP of this resume list because they gate Phase 11 hand-off.
...
7. **Phase 11-A — Repo push + conventional commits** (brief §10) — Squash our local work into conventional-commit messages (`feat:`, `fix:`, `perf:`, `docs:`, `a11y:`); push to `github.com/j0vis/ktube`; document in RELEASING.md. ~ 1 hour. Gated on operator permission. **NOT auto-doable — needs explicit go-ahead.**
7. **Phase 11-A — Repo push + conventional commits** (brief §10) — Squash our local work into conventional-commit messages (`feat:`, `fix:`, `perf:`, `docs:`, `a11y:`); push to `github.com/j0vis/ktube`; document in RELEASING.md. ~ 1 hour. Gated on operator permission. **NOT auto-doable — needs explicit go-ahead.**
...
9. **Customizer hardening followups** — add `'enqueued'` check alongside `'registered'` in `ktube_print_inline_customizer_vars()` so a MU-plugin dequeue doesn't orphan inline CSS. (~ 15 minutes; already shipped as belt-and-suspenders in `PrintInlineCustomizerVarsTest` review — verify on merge.)
10. **Schema richness** — add `articleBody` (from `wp_strip_all_tags the_content`) + `keywords` (from `post_tag` terms) to `ktube_render_blog_article_schema`; decide whether `og:type="video.other"` is the right mapping for `is_singular('video')`. ~ 2 hours.
11. **Player depth** — quality/bitrate selection UI; `videojs-http-streaming` lazy-import behind `Hls.isSupported()` probe behind a Customizer flag (re-opens Phase 3b's HLS trade-off). ~ 1 day.
12. **Phase 11 — Customizer audit** — unit-test `ktube_register_customizer` boot path, snapshot-test rendered Customizer JSON, trace `customize_save_after` hook to verify theme_mod persistence. ~ 1 day.
13. **Phase 14 — Performance pass** — defer lightbox CSS (only enqueued on photo singles), image-format negotiation (`ktube-card.avif` with `jpg` fallback), critical CSS inlining for above-the-fold. ~ 1 day.
14. **Phase 9b Privacy disclosure followups** — single remaining PHPUnit `assertSame` failure (`Expected 42 / Actual 7` in `test_resolve_privacy_page_id_prefers_configured_over_slug_fallback`) after the WP_Query stub fix to honor `post_type`/`post_status`/`name` filters. Root cause: configured-branch's `get_post()` returning `null` for ID 42 even after `set_post_test(42, ...)` was called. ~ 15 minutes.

After §5-A ships and §5-B is closed out, the theme is ready for Phase 11 hand-off and can re-rebase against `feat/customizer-handling` for the Phase 0-A tech-stack reversal.
2. **Customizer hardening** — **shipped 2026-06-21.** `ktube_print_inline_customizer_vars()` now gates on BOTH `wp_style_is('ktube-main','registered')` AND `wp_style_is('ktube-main','enqueued')` (belt-and-suspenders; enqueue implies register in WP semantics but the dual gate is defensive and locks the test). Bootstrap stubs now controllable via `set_style_state_test()`, plus recording stubs for `wp_add_inline_style` / `wp_add_inline_script`. New `tests/phpunit/ktube/PrintInlineCustomizerVarsTest.php` covers 8 cases: pipe-when-enqueued/registered, noop-when-only-registered, noop-when-only-enqueued, noop-when-neither, root-block-token-presence, customizer-settings-echoed-in-css, sha256-hex-per-key-checksums, out-of-range-clamp-on-read.
3. **Player depth** — quality/bitrate selection UI; `videojs-http-streaming` lazy-import behind `Hls.isSupported()` probe behind a Customizer flag (re-opens Phase 3b's HLS trade-off). ~ 1 day.
4. **Dark-mode cosmetic cleanup** — **shipped 2026-06-21.** `ktube_enqueue_dark_mode` in `includes/setup.php` no longer `wp_add_inline_scripts` `window.ktubeThemeData = { default: ... }`. The Customizer preview iframe's separate emission via `ktube_enqueue_customize_controls` is preserved (controls.js needs the default at load). Doc comment in setup.php now explains the precedence (`ktube_inline_theme_bootstrap` at `wp_head` priority -1 sets `<html data-theme>` server-side, so dark-mode.js::`getTheme()` reads the resolved value with no localization needed).
5. **Phase 11 — Customizer audit** — unit-test `ktube_register_customizer` boot path, snapshot-test rendered Customizer JSON, trace `customize_save_after` hook to verify theme_mod persistence. ~ 1 day.
6. **Phase 14 — Performance pass** — defer lightbox CSS (only enqueued on photo singles), image-format negotiation (`ktube-card.avif` with `jpg` fallback), critical CSS inlining for above-the-fold. ~ 1 day.
7. **Phase 10 — CI matrix** — `.github/workflows/ci.yml` matrix over PHP 8.3 + Node 20 running `composer install` / `npm run build` / `npm test` / `npm run test:phpunit` / `npm run validate`. Currently `.github/workflows/build-enforcement.yml` validates the manifest post-build; the test matrix itself is still pending. ~ 2 hours.
8. **Phase 9b Privacy disclosure followups** — single remaining PHPUnit `assertSame` failure (`Expected 42 / Actual 7` in `test_resolve_privacy_page_id_prefers_configured_over_slug_fallback`) after the WP_Query stub fix to honor `post_type`/`post_status`/`name` filters. Root cause is the configured-branch's `get_post()` returning `null` for ID 42 even after `set_post_test(42, …)` was called — the static-`self::$passes` counter on the stub TestCase was last bumped by the failing assertion, suggesting the GLOBAL state for posts is intact but the function chain isn't reading it. Candidate fix: ensure `__ktube_posts` is consulted before `__ktube_theme_mods` in the configured branch by inlining the post lookup, or expose a `ktube_test_get_post` shim in test context. ~ 15 minutes.

After that, all reviewer nits are closed and Phase 10–14 are ready for hand-off.
...
| VideoObject JSON-LD schema | Article, ImageGallery, OG/Twitter Card — but **no VideoObject** on video singles | `schema.org/VideoObject` with `contentUrl`, `thumbnailUrl`, `uploadDate`, `duration`, `encodingFormat`, `interactionStatistic` | schema.php extension — closed under §5.1 Schema richness followup |
...
| Age-gate cascading jurisdictions | Single age-gate setting (any-country) | IP-based jurisdictional gating (`KTUBE_AGE_GATE_BLOCKED_COUNTRIES` list); RTA meta + per-CO redirect | §6 (Phase 6 deep) |
...
| Schema markup for video Carousel eligibility | OG image present; no `VideoObject` → no video-rich-result in SERP | Add structured `VideoObject` first; then add `itemListElement` for channel/actor archive pages | §5.1 + new Phase 16 (Video SERP enrichment) |
